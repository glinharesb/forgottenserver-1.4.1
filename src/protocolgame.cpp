...
void ProtocolGame::login(const std::string& name, uint32_t accountId, OperatingSystem_t operatingSystem)
{
	...
	if (!foundPlayer || g_config.getBoolean(ConfigManager::ALLOW_CLONES) || name == "Account Manager") {
		...
		if (g_config.getBoolean(ConfigManager::ONE_PLAYER_ON_ACCOUNT) && player->getAccountType() < ACCOUNT_TYPE_GAMEMASTER && g_game.getPlayerByAccount(player->getAccount()) && !player->isAccountManager()) {
...
void ProtocolGame::onRecvFirstMessage(NetworkMessage& msg)
{
	...
	if(characterName == "Account Manager" && g_config.getBoolean(ConfigManager::ACCOUNT_MANAGER)) {
		IOLoginData::getAccountId(accountName, accountId);
	}
